...
pub fn print_info(config: &Configuration, info: &mut SystemInfo) -> io::Result<()> {
    ...
    if config.show_os {
        println!("{}\x1b[0m\x1b[1mOWOS     \x1b[0m{}", 
            move_cursor,
            info.os_name
        );
    }
    ...
    if config.show_host {
        println!("{}\x1b[0m\x1b[1mMOWODEL  \x1b[0m{}",
            move_cursor,
            info.model
        );
    }
    ...
    if config.show_kernel {
        println!("{}\x1b[0m\x1b[1mKEWNEL   \x1b[0m{}",
            move_cursor,
            info.kernel
        );
    }
    ...
    if config.show_cpu {
        println!("{}\x1b[0m\x1b[1mCPUWU    \x1b[0m{}",
            move_cursor,
            info.cpu_model
        );
    }
    ...
    if config.show_gpu {
        for gpu in &info.gpu_models {
            println!("{}\x1b[0m\x1b[1mGPUWU    \x1b[0m{}",
                move_cursor,
                gpu
            );
        }
    ...
    if config.show_ram {
        println!("{}\x1b[0m\x1b[1mMEMOWY   \x1b[0m{} MiB/{} MiB",
            move_cursor,
            info.ram_used,
            info.ram_total
        );
    ...
    if config.show_resolution {
        if info.screen_width != 0 || info.screen_height != 0 {
            println!("{}\x1b[0m\x1b[1mWESOWUTION\x1b[0m  {}x{}",
                move_cursor,
                info.screen_width,
                info.screen_height
            );
    ...
    if config.show_shell {
        println!("{}\x1b[0m\x1b[1mSHEWW    \x1b[0m{}",
            move_cursor,
            info.shell
        );
    }
    ...
    if config.show_pkgs {
        println!("{}\x1b[0m\x1b[1mPKGS     \x1b[0m{}: {}",
            move_cursor,
            info.pkgs,
            info.pkgman_name
        );
    ...
    if config.show_uptime {
        ...
        println!("{}\x1b[0m\x1b[1mUWUPTIME \x1b[0m{}",
            move_cursor,
            uptime_str
        );
    }
...
fn format_uptime(seconds: u64) -> String {
    match seconds {
        ...
        _ => format!("{}d, {}h, {}m", seconds / 86400, seconds / 3600 % 24, seconds / 60 % 60),
    }
}
...
fn find_ascii_file(os_name: &str) -> Option<String> {
    ...
    let system_path = format!("/data/data/com.termux/files/usr/lib/uwufetch/ascii/{}.txt", os_name);
...
pub fn print_image(info: &SystemInfo) -> io::Result<usize> {
    let image_path = if let Some(ref custom_image) = info.image_name {
        ...
        #[cfg(target_os = "android")]
        let default_path = format!("/data/data/com.termux/files/usr/lib/uwufetch/{}.png", info.os_name);
